    ...
    const fetchDataForWorker = async () => {
      ...
      try {
        ...
        const [availRes, appoRes] = await Promise.all([
          ...
          fetchData(
            `appointment/getAdminAppoiment/${workerId}`,
            'get',
            null,
            token,
          ),
  ...
  */
  useEffect(() => {
    ...
    const availEvents = schedule
      .filter(
        (item) =>
          Number(item.day_id) === Number(selectedDayId) &&
          item.start_time &&
          item.end_time,
      ) // filtramos por día y tiempos válidos
      .map((item) => {
        ...
        const start = dayjs(getDateFromDayId(item.day_id, date))
          .hour(sh)
          .minute(sm)
          .second(0);
        const end = dayjs(getDateFromDayId(item.day_id, date))
          .hour(eh)
          .minute(em)
          .second(0);
    ...
    const appointmentEvents = appointments
      .map((item) => {
        ...
        const e =
          parseDateTime(item.appointment_date, item.end_time) ||
          (s ? s.add(Number(item.duration_minutes || 0), 'minute') : null); // fin o derivado
        return s && e
          ? {
    ...
    const durationMinutes = Math.round(
      sumaTotalMinutos + Number(cleaningServiceDuration || 0),
    ); // duración requerida en minutos (incluye limpieza)
    ...
    const overlapsFn = (startA, endA, startB, endB) =>
      startA < endB && startB < endA; // función de solapamiento
    ...
    mergedAvail.forEach((avail) => {
      ...
      if (alignedMinute !== startMinute)
        cursor = cursor.minute(alignedMinute).second(0);

      // Mientras que haya espacio para la duración requerida dentro de la franja
      while (
        cursor.add(durationMinutes, 'minute').valueOf() <= availEnd.valueOf()
      ) {
        ...
        const clashes = appointmentEvents.some((app) =>
          overlapsFn(
            candidateStart.toDate(),
            candidateEnd.toDate(),
            app.start,
            app.end,
          ),
        ...
        if (!clashes) {
          ...
          slots.push({
            start: candidateStart.toDate(),
            end: candidateEnd.toDate(),
            label: candidateStart.format('HH:mm'),
          });
        }
  ...
  const handleConfirm = async () => {
    ...
    const supplementIdsPayload = Array.isArray(extrasIds)
      ? extrasIds.map((n) => Number(n)).filter((n) => !Number.isNaN(n))
      : [];
    const baseServiceIdNum = baseServiceId ? Number(baseServiceId) : null;
    const cleaningServiceIdNum = cleaningServiceId
      ? Number(cleaningServiceId)
      : null;

    if (
      cleaningServiceIdNum &&
      !supplementIdsPayload.includes(cleaningServiceIdNum) &&
      cleaningServiceIdNum !== baseServiceIdNum
    ) {
  ...
  /* Función para no permitir la selección de cita en un día ya pasado */
    ...
    <>
          ...
          {workers?.map((elem) => (
            ...
            >
              <div className="appointmentEmployeeCard">
                {elem.picture_user &&
                elem.picture_user !== '' &&
                elem.picture_user !== 'IconDefect.png' ? (
                  <img
      ...
      <div className="selectEmployeeActions">
        <div className="infoAppoint">
          <h3>
            <img src="/img/appointment/clock.png" />
            {minutesToHour(sumaTotalMinutos)}
          </h3>
          <h3>
            <img src="/img/appointment/cart.png" />
            {sumaTotalPrecio.toFixed(2)}€
          </h3>
        </div>
        <button
          type="button"
